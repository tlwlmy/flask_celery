...
class User(Base):
    # 用户
    __tablename__ = 'user'
    uid = Column('uid', Integer, primary_key=True)    # 用户id
    name = Column('name', String(64))    # 用户名
    password = Column('password', String(32), default='')    # 用户密码
    create_time = Column('create_time', DateTime, default=datetime.now)    # 创建时间
    last_edit = Column('last_edit', DateTime, default=datetime.now)    # 最后编辑时间
    ...
    def update(record, modify_info):
        # 更新
        affected_row = db.query(User).filter(User.uid==record['uid']).update(modify_info)
